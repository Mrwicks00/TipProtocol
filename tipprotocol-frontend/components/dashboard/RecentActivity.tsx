...
export function RecentActivity({ activities }: RecentActivityProps) {
  ...
  return (
    <Card className="bg-card border-border">
      <CardHeader>
        <CardTitle className="text-card-foreground text-lg sm:text-xl">Recent Activity (Live Events)</CardTitle>
        <CardDescription>Your latest transactions (only shows new events while page is open)</CardDescription>
      </CardHeader>
      <CardContent>
        {activities.length === 0 ? (
          <p className="text-muted-foreground text-center py-4">
            No recent activity detected. Try making a transaction!
          </p>
        ) : (
              ...
              .map((activity, index) => (
                ...
                >
                  <div className="flex items-center space-x-4">
                    <div
                      className={`w-10 h-10 rounded-full flex items-center justify-center ${getActivityBg(activity.type)}`}
                    >
                      {getActivityIcon(activity.type)}
                    </div>
                    <div className="min-w-0 flex-1">
                      <p className="font-medium text-card-foreground">{getActivityText(activity)}</p>
                  ...
                  <div className="flex flex-col sm:flex-row items-start sm:items-center gap-2 sm:gap-4">
                    <div className="text-left sm:text-right">
                      <p className="font-medium text-card-foreground">
                        ${Number.parseFloat(activity.amount).toFixed(2)}
                      </p>
